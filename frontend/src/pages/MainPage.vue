...
<script setup>
    ...
    const filteredTopics = computed(() => {
        return topicTagData.value.filter(topic => {
            const query = searchQuery.value?.toLowerCase() || ''
            const matchesSearch = topic.title.toLowerCase().includes(query)

            let matchesTags = true
            if (selectedTags.value.length > 0) {
                const topicTagIds = Array.isArray(topic.Tags) ? topic.Tags.map(tag => tag.id) : []
                // console.log(topicTagIds)

                matchesTags = selectedTags.value.every(tagId => topicTagIds.includes(tagId))
            }
    ...
    const filteredBlocks = computed(() => {
        const blocks = Array.isArray(blockData.value)
            ? blockData.value.map(block => ({
                  ...block,
                  topics: [],
              }))
            : []
