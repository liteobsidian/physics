<template>
    <div class="editor-container">
        <div class="d-flex justify-sm-space-between">
            <h3 class="editor-font">Форма добавления нового задания</h3>
            <v-btn @click="($emit('update:selectedExerciseOption', null), cleanFields())" width="20%">Отмена</v-btn>
        </div>
        <span class="editor-description"
            >Здесь вы можете добавить новое задание. Для того чтобы оно добавилось вам нужно заполнить все поля и
            выбрать тему, к которой будет принадлежать будущее задание, вместе с типом</span
        >
        <div class="d-flex admin-task-editor-buttons">
            <v-btn
                @click="((onAddImage = !onAddImage), (onAddText = false), (taskText = null))"
                width="40%"
                height="3rem"
                style="align-self: center"
                >Добавить изображение</v-btn
            >
            <span>или</span>
            <v-btn
                @click="((onAddText = !onAddText), (onAddImage = false), (taskImage = null))"
                width="40%"
                height="3rem"
                style="align-self: center"
                >Добавить текст</v-btn
            >
        </div>

        <div v-show="onAddImage">
            <v-file-input
                label="Внесите изображение для задания"
                accept="image/*"
                v-model="taskImage"
                variant="underlined"
            ></v-file-input>
        </div>
        <div v-show="onAddText">
            <v-textarea
                label="Введите текст задания"
                v-model="selectedTaskText"
                content-type="html"
                variant="filled"
                autocomplete="off"
                auto-grow
            />
        </div>
        <div>
            <h3 class="editor-font">Ответ:</h3>
            <v-text-field label="Введите ответ к заданию" v-model="selectedAnswer" autocomplete="off"></v-text-field>
        </div>
        <div class="d-flex" style="flex-direction: column">
            <h3 class="editor-font">Подсказка:</h3>
            <h5 style="color: #7d7d7d">Можно сделать подсказку либо с тестом, либо с изображением</h5>
            <v-text-field
                label="Введите подсказку"
                v-model="selectedHintText"
                @click="taskHintImage = null"
                autocomplete="off"
            ></v-text-field>
            <span class="text-center" style="color: #7d7d7d">или</span>
            <v-file-input
                label="Внесите изображение для подсказки"
                accept="image/*"
                v-model="taskHintImage"
                variant="underlined"
                @click="selectedHintText = null"
            ></v-file-input>
        </div>
        <div>
            <h3 class="editor-font">Тема:</h3>
            <v-autocomplete
                v-model="selectedTopic"
                :items="props.topics"
                item-title="title"
                item-value="id"
                label="Выберите тему"
                return-object
                autocomplete="off"
            />
        </div>
        <div>
            <h3 class="editor-font">Тип задания:</h3>
            <v-autocomplete
                v-model="selectedType"
                :items="props.taskTypes"
                item-title="title"
                item-value="type"
                label="Выберите тип задания"
                return-object
            />
        </div>
        <div class="d-flex" style="justify-content: space-between">
            <v-dialog max-width="800">
                <template v-slot:activator="{ props: activatorProps }">
                    <v-btn v-bind="activatorProps" color="#4285f4" text="Посмотреть задание" variant="flat"></v-btn>
                </template>

                <template v-slot:default="{ isActive }">
                    <v-card :title="`Просмотр задания типа: ${selectedType?.title || 'Не выбрано'}`">
                        <v-card-text>
                            <span class="mb-4">Выбранная тема: {{ selectedTopic?.title || 'Не выбрано' }}</span>
                            <div v-if="isImage(taskImage)" class="d-flex mb-4 mt-4" style="justify-content: center">
                                <v-img
                                    v-if="taskImageUrl"
                                    :src="taskImageUrl"
                                    class="mx-auto"
                                    max-width="100%"
                                    contain
                                />
                            </div>
                            <div v-else class="text-body-1 mb-4 mt-4 task-text" v-html="selectedTaskText"></div>
                            <v-expansion-panels class="mb-4 hint-panel">
                                <v-expansion-panel density="compact" class="hint-panel-outline" elevation="0">
                                    <v-expansion-panel-title>
                                        <v-icon size="small" color="amber-darken-2" class="me-2"
                                            >mdi-lightbulb-outline</v-icon
                                        >
                                        <span class="text-body-2">Подсказка</span>
                                    </v-expansion-panel-title>
                                    <v-expansion-panel-text class="text-body-2">
                                        <!-- Подсказка с изображением -->
                                        <div v-if="isImage(taskHintImage)" class="text-center">
                                            <v-img
                                                v-if="taskHintImageUrl"
                                                :src="taskHintImageUrl"
                                                max-width="100%"
                                                contain
                                                class="mx-auto"
                                            ></v-img>
                                        </div>
                                        <!-- Подсказка с HTML -->
                                        <div v-else v-html="selectedHintText"></div>
                                    </v-expansion-panel-text>
                                </v-expansion-panel>
                            </v-expansion-panels>
                            <span>Ответ: {{ selectedAnswer }}</span>
                        </v-card-text>

                        <v-card-actions>
                            <v-spacer></v-spacer>

                            <v-btn text="Вернуться к редактированию" @click="isActive.value = false"></v-btn>
                            <v-btn
                                style="background-color: #4285f4"
                                color="#ffff"
                                text="Добавить задание"
                                @click="(addTask(), (isActive.value = false))"
                            ></v-btn>
                        </v-card-actions>
                    </v-card>
                </template>
            </v-dialog>
        </div>
    </div>
</template>
<script setup>
    import { ref, defineProps, defineEmits, watch } from 'vue'
    import '@/assets/main.css'
    import { addTaskAdmin } from '@/services/admin.service'

    const props = defineProps({
        addTask: Function,
        topics: Array,
        taskTypes: Array,
        selectedExerciseOption: String,
        errorEditingTask: { type: Object, default: () => ({}) },
        successEditingTask: { type: Object, default: () => ({}) },
        getData: Function,
    })

    const taskImage = ref(null)
    const taskHintImage = ref(null)
    const taskImageUrl = ref(null)
    const taskHintImageUrl = ref(null)
    const selectedTopic = ref(null)
    const selectedType = ref(null)
    const selectedAnswer = ref('')
    const selectedHintText = ref('')
    const selectedTaskText = ref('')

    const taskImgBuffer = ref('')
    const hintImgBuffer = ref('')

    const emit = defineEmits(['update:selectedExerciseOption', 'update:errorEditingTask', 'update:successEditingTask'])

    watch([taskImage, taskHintImage], ([newTaskImage, newTaskHintImg]) => {
        if (newTaskImage && newTaskImage instanceof File) {
            const reader1 = new FileReader()
            reader1.onload = e => {
                taskImageUrl.value = URL.createObjectURL(newTaskImage)
                const base64 = e.target.result.split(',')[1]
                taskImgBuffer.value = base64
            }
            reader1.readAsDataURL(newTaskImage)
        } else {
            taskImgBuffer.value = null
        }

        if (newTaskHintImg && newTaskHintImg instanceof File) {
            const reader2 = new FileReader()
            reader2.onload = e => {
                taskHintImageUrl.value = URL.createObjectURL(newTaskHintImg)
                const base64 = e.target.result.split(',')[1]
                hintImgBuffer.value = base64
            }
            reader2.readAsDataURL(newTaskHintImg)
        } else {
            hintImgBuffer.value = null
        }
    })
    const isImage = task => {
        if (task instanceof Blob) {
            return true
        } else {
            return false
        }
    }

    const cleanFields = () => {
        ;(selectedTaskText.value = ''),
            (taskImgBuffer.value = ''),
            (selectedTopic.value = null),
            (selectedAnswer.value = ''),
            (selectedHintText.value = ''),
            (hintImgBuffer.value = ''),
            (selectedType.value = null)
    }

    async function addTask() {
        try {
            const responese = await addTaskAdmin(
                !taskImgBuffer.value ? selectedTaskText.value : taskImgBuffer.value,
                selectedAnswer.value,
                !hintImgBuffer.value ? selectedHintText.value : hintImgBuffer.value,
                Number(selectedTopic.value?.id),
                selectedType.value?.type,
            )
            if (responese?.status == 201) {
                emit('update:successEditingTask', {
                    show: true,
                    text: 'Задание усепшно добавлено',
                })
                await props.getData()
                cleanFields()
                emit('update:selectedExerciseOption', null)
                return
            }
        } catch (error) {
            emit('update:errorEditingTask', {
                show: true,
                text: `Ошибка: ${error}. Задание не добавлено`,
            })
            cleanFields()
            console.log(error)
            emit('update:selectedExerciseOption', null)
            return
        }
    }

    const onAddImage = ref(false)
    const onAddText = ref(false)
</script>
<style scoped lang="scss">
    .admin-choice {
        display: flex;
        align-items: center;
        gap: 1rem;
        justify-content: space-between;
    }
    .text-area {
        background-color: #ececec;
        border-bottom: 1px solid #9d9d9d;
        width: 70%;
        height: 2rem;
        text-align: center;
    }
    .admin-task-editor-buttons {
        flex-direction: column;
        justify-content: center;
        text-align: center;
        gap: 0.5rem;
    }
    .check-task-admin {
        width: 100%;
    }
    .hint-panel-outline {
        border: 1px solid rgba(0, 0, 0, 0.12) !important;
    }
    .hint-panel {
        max-width: 100%;
    }
    .task-tags-admin {
        flex-wrap: wrap;
        max-height: 180px;
        overflow-y: auto;
        gap: 5px;
    }
    .mb-4 {
        margin-bottom: 1.5rem;
    }
    .mt-4 {
        margin-top: 1.5rem;
    }
</style>
